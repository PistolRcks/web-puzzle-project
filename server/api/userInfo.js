...
function setUserPFP(req, res, next) {
  ...
  // Type validation
  const { profile_picture, profile_picture_top, profile_picture_left } =
    req.body;
  ...
  // Test that profile_picture is base64 actually (regex may not be specific enough, but good enough for right now)
  if (!/^([a-z]|[A-Z]|[0-9]|[+/=])+$/.test(profile_picture)) {
    res.status(400).send("Error: 'profile_picture' is not Base64-encoded!");
    return;
  }
  ...
  // Input data now
  db.run(
    "UPDATE User SET profile_picture = ?, profile_picture_top = ?, profile_picture_left = ? WHERE user_id = ?",
    [
      profile_picture,
      profile_picture_top,
      profile_picture_left,
