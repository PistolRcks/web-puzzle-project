...
const redirectBundleManifest = (req, res, next) => {
  const redirectFileNames = ['bundle.js', 'bundle.css', 'manifest.json', 'favicon.ico']
...
const logRouteAndCheckAuthorization = (req, res, next) => {
  const exceptedRoutes = ['/api/login', '/', '/bundle.css', '/bundle.js', '/manifest.json', '/logo192.png', './favicon.ico']
  ...
  if (!exceptedRoutes.includes(req.path) && !req.session?.userID && !res.headersSent) {
