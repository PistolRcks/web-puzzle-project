...
describe("Test insertUser", () => {
    ...
    test("Standard user: output of insertUser", (done) => {
        ...
        Crypto.pbkdf2("password0", salt, 310000, 32, 'sha256', function(err, hashedPassword) {
    ...
    test("Standard user: username found in database", (done) => {
        ...
        Crypto.pbkdf2("password1", salt, 310000, 32, 'sha256', async function(err, hashedPassword) {
            insertUser(db, "username1", hashedPassword, salt, async () => {
                ...
                db.get(`select username as u from User where username="username1"`, function(err, row) {
                    if (err) throw err;
    ...
    test("Standard user: salt found in database", (done) => {
        ...
        Crypto.pbkdf2("password2", salt, 310000, 32, 'sha256', function(err, hashedPassword) {
            insertUser(db, "username2", hashedPassword, salt, () => {
                db.get(`select salt as s from User where username="username2"`, function(err, row) {
                    if (err) throw err;
